...
```jsonc
{
  "UVision": {
    ...
    "Ml": {
      "Provider": "none",      // none(VLM 단독) | mock | mloop — 전용 ML 분류기(신뢰성 플라이휠 ②~③)
      "Endpoint": "",          // mloop serve base URL(예: http://host:5000)
      "Model": "default",      // 학습 모델 이름
      "Token": "",             // mloop serve JWT Bearer
      "ImageColumn": "ImagePath",
      "ReviewConfidenceThreshold": 0.0  // ③ 2중체크: VLM/ML 신뢰도가 이 값 미만이면 requires_review(0=불일치만)
    },
...
`Ml.Provider` 가 `none` 이 아니면 `/inspect` 가 VLM·ML 을 **병렬 호출**(`Task.WhenAll`)해
교차검증하고, 응답·저장 레코드에 **additive 필드**를 덧붙인다(ML `none`이면 필드 전체 생략 = 기존 wire 보존):

```jsonc
{
  "verdict": "OK", "findings": "", "confidence": 0.95, "timestamp": "...", "image_id": "...",
  "ml": { "label": "ok", "confidence": 0.98 },  // 전용 ML 교차검증
  "agreement": true,                              // VLM·ML 일치 여부
  "requires_review": false                        // 불일치/저신뢰 → 사람·오라클 검토(③→④)
}
```

VLM verdict 가 여전히 주 판정이고 ML 은 교차검증이다. ML 호출 실패는 판정을 막지 않는다(degrade — VLM 단독 응답).
...
| PUT/DELETE/GET | `/results/label`, `/results/labels` (사람 OK/NG 라벨) | - |
| GET | `/datasets`, `/datasets/{exportId}/manifest` | - |
| POST | `/datasets/export` (라벨→ML 학습입력 스냅샷) | PIN |
